...
**Audio Digest**: `agent/audio_digest.py` generates a spoken-word audio file from the briefing digest for listening while driving or walking. Two styles via `audio_style` setting: **Q&A** (default) produces a Host/Expert conversation where the Expert references the user's active SCMS projects by name — `_fetch_project_context()` pulls project names, descriptions, goals, and stack from SCMS. **Monologue** produces a straight narrated read-through. Q&A mode uses dual voices (`tts_voice_host` = am_michael, `tts_voice_expert` = af_bella) via `split_qa_by_speaker()` which routes each speaker's segments to the appropriate Kokoro voice. Q&A prefers the deep-dive markdown for richer content; monologue uses the briefing. `_clean_script()` strips markdown artifacts (bold, italic, links, URLs) before TTS to prevent Kokoro from reading formatting aloud. TTS: Kokoro locally (82M params, CPU, free) with OpenAI TTS fallback. Output: MP3 saved to digests directory. Requires `ffmpeg` for MP3 encoding (falls back to WAV).
